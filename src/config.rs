...
use std::{io::{Cursor, Read, Write}, error::Error};
...
impl FT60xConfig {
    pub fn parse(bytes: [u8; 152]) -> Result<FT60xConfig, Box<dyn Error>> {
        ...
        if data.read(&mut strings_buf)? != 128 {
            Err(Ft60xError::Unknown)?;
        }

        fn parse_string(bytes: &[u8]) -> Result<(String, u8), Box<dyn Error>> {
            let offset = bytes[0];
    ...
    pub fn encode(&self) -> Result<[u8; 152], Box<dyn Error>> {
        let mut buf = [0; 152];
        ...
        fn encode_string(string: &str, cursor: &mut Cursor<&mut [u8]>) -> Result<(), Box<dyn Error>> {
...
impl FT60xFifoMode {
    fn parse(num: u8) -> Result<Self, Box<dyn Error>> {
        match num {
            0 => Ok(Self::Mode245),
            1 => Ok(Self::Mode600),
            _ => Err(Ft60xError::InvalidFifoMode)?,
        }
    }
...
impl FT60xFifoClock {
    fn parse(num: u8) -> Result<Self, Box<dyn Error>> {
        match num {
            ...
            _ => Err(Ft60xError::InvalidFifoClock)?,
        }
    }
...
impl FT60xChannelConfig {
    fn parse(num: u8) -> Result<Self, Box<dyn Error>> {
        match num {
            ...
            _ => Err(Ft60xError::InvalidChannel)?,
        }
    }
...
pub mod ft60x_flash_rom_detection {
    ...
    impl FT60xFlashRomDetection {
        pub fn parse(flags: u8) -> Result<FT60xFlashRomDetection, Box<dyn Error>> {
